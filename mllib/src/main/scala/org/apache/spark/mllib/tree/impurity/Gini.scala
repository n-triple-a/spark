 ...
 */
private[spark] class GiniAggregator(numClasses: Int, classWeights: Array[Double])
  ...
  def getCalculator(allStats: Array[Double], offset: Int): GiniCalculator = {
    new GiniCalculator(allStats.view(offset, offset + statsSize).toArray, classWeights)
  }
}
 ...
 * @param classWeights Weights of classes
 */
private[spark] class GiniCalculator(stats: Array[Double], classWeights: Array[Double])
   ...
   */
  def copy: GiniCalculator = new GiniCalculator(stats.clone(), classWeights.clone())

  /**
   * Calculate the impurity from the stored sufficient statistics.
   */
  def calculate(): Double = Gini.calculate(weightedStats, weightedStats.sum)
  ...
  override def add(other: ImpurityCalculator): ImpurityCalculator = {
    ...
    val otherCalculator = other.asInstanceOf[GiniCalculator]
    var i = 0
